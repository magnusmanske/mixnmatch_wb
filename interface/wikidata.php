...
class WDI {
	...
	public $q ;
	public $j ;
	public $wikidata_api_url ;
	
	public function WDI ( $q = '' , $wikidata_api_url = 'https://www.wikidata.org/w/api.php' ) {
		$this->wikidata_api_url = $wikidata_api_url ;
		if ( $q != '' ) {
			$q = 'Q' . preg_replace ( '/\D/' , '' , "$q" ) ;
			$this->q = $q ;
			$url = "{$this->wikidata_api_url}?action=wbgetentities&ids=$q&format=json" ;
...
class WikidataItemList {
	...
	public $wikidata_api_url = 'https://www.wikidata.org/w/api.php' ;
	...
	protected function parseEntities ( $j ) {
		foreach ( $j->entities AS $q => $v ) {
			...
			$this->items[$q] = new WDI ( '' , $this->wikidata_api_url );
			$this->items[$q]->q = $q ;
			$this->items[$q]->j = $v ;
		}
	}
    ...
    function loadItems ( $list ) {
    	...
    	foreach ( $qs AS $k => $sublist ) {
			...
			$url = "{$this->wikidata_api_url}?action=wbgetentities&ids=" . implode('|',$sublist) . "&format=json" ;
			$urls[$k] = $url ;
    	}
#print_r ( $urls ) ;
	...
	public function loadItemByPage ( $page , $wiki ) {
		...
		$url = $this->wikidata_api_url . "?action=wbgetentities&sites=$wiki&titles=$page&format=json" ;
		$j = json_decode ( file_get_contents ( $url ) ) ;
